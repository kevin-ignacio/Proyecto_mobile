...
export class RegistroPage   {
  ...
  constructor(private router: Router,private locationService:LocationService,public fb: FormBuilder, public alertController: AlertController) {
    this.formularioRegistro = this.fb.group({
      ...
      'nombreLogin': new FormControl("", [Validators.required, Validators.minLength(3), Validators.maxLength(8), Validators.pattern(/^[a-zA-Z0-9]*$/)]),
  ...
  async guardar() {
    ...
    if (this.usuarioExiste(usuarios, f.nombreLogin)) {
    ...
    if (f.nombreLogin.length < 3 || f.nombreLogin.length > 8) {
